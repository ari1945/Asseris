...
| ⬜ | **14 view sisa (Fase 2).** 2-imp: `view_execution` `view_firm` `view_groupaudit` `view_palette` `view_procurement` `view_wp` · 3-imp: `view_cockpit` `view_materiality` | 2–3 | **W12 Fase 2** (`view_firm`/`view_procurement`/`view_groupaudit`/`view_materiality` punya shim `.d.ts` → hapus saat konversi) |
| ⬜ | penyedia-bersama: `view_docparts`(7) `view_onboarding`(9) `view_bo1`(12) `view_calc`(22) `view_fpm_parts`(29) `view_analytical`(48) | 7–48 | **W12 Fase 2** (TERAKHIR; hapus shim `.d.ts`-nya saat konversi) |
