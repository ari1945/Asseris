...
function DFAturan() {
  ...
  const err = RULES.filter(r => r.status === 'err').length;
  const byCat = Array.from(new Set(RULES.map(r => r.cat))).map(c => ({ c, n: RULES.filter(r => r.cat === c).length, ok: RULES.filter(r => r.cat === c && r.status === 'pass').length }));
      ...
      <Panel noBody>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="dtbl" style={{ minWidth: 720 }}>
            <thead><tr><th style={{ minWidth: 200 }}>Field Master</th>{modules.map(m => <th key={m.id} className="num" style={{ width: 86 }}>{m.label}</th>)}</tr></thead>
            <tbody>
              {fields.map(f => (
                <tr key={f}>
...
function DFJejak() {
  ...
  const live = (logEntries || []).map(l => ({ ts: l.ts, user: 'Anindya P.', action: l.action || l.what || 'Aktivitas', entity: l.entity || l.detail || '', module: l.module || '—' }));
  const all = [...live, ...TRAIL];
  const mods = ['All', ...Array.from(new Set(all.map(t => t.module)))];
  const rows = mod === 'All' ? all : all.filter(t => t.module === mod);

  const byUser = Object.values(all.reduce((m, t) => { m[t.user] = m[t.user] || { user: t.user, n: 0 }; m[t.user].n++; return m; }, {})).sort((a, b) => b.n - a.n);
  ...
  return (
    <div className="view-scroll"><div className="view-pad">
      <div className="grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 12 }}>
        ...
        <Panel><div style={{ padding: '11px 14px' }}><Stat value={all.filter(t => (t.ts || '').startsWith('2026-03-05')).length} label="Hari Ini" accent="var(--blue)" /></div></Panel>
        <Panel><div style={{ padding: '11px 14px' }}><Stat value={new Set(all.map(t => t.module)).size} label="Modul Tersentuh" /></div></Panel>
      ...
      <div className="grid" style={{ gridTemplateColumns: '2fr 1fr', gap: 12, alignItems: 'start' }}>
        <Panel noBody>
          <div className="panel-h"><h3>Jejak Audit (Immutable Log)</h3><div style={{ flex: 1 }} /><Seg options={mods.map(m => ({ value: m, label: m === 'All' ? 'Semua' : m }))} value={mod} onChange={setMod} /></div>
        ...
        <Panel title="Aktivitas per Pengguna">
          <div style={{ padding: 14 }}>
            <HBars rows={byUser.map(u => ({ label: u.user, value: u.n, right: u.n + ' aksi', color: 'var(--navy)' }))} />
