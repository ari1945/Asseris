...
import type { FirmInvoice } from './use_firm_subledger';
...
function FirmAPAR() {
  ...
  /* A2 — piutang dibaca dari register yang DITULIS modul Billing
     (`useAmsPersist('invoices')`, view_pipeline.tsx), bukan dari seed `AMS.INVOICES`.
     Dulu keduanya register terpisah untuk satu konsep: menerbitkan faktur, menandainya
     lunas, atau mengubah jatuh temponya di Billing tidak terlihat sama sekali di layar
     ini — padahal di sinilah Piutang Outstanding, DSO, Posisi Neto dihitung, dan di
     sinilah baris rekonsiliasi `1-200` dimiliki (`owner: 'apar'`).
     Dibaca langsung, BUKAN lewat `useFirmSubledger()`: hook itu juga membuka
     `useAmsPersist('firmap')`, dan komponen ini sudah memegang instance TULIS-nya
     sendiri di atas — dua salinan state yang hidup bersamaan bisa menyimpang. */
  const [ar] = useAmsPersist('invoices', () => AMS.INVOICES);
  const REF = new Date(AMS.TODAY); /* K-02: klok SSOT */
  ...
  const arOverdue = ar.filter((x: FirmInvoice) => x.status === 'Overdue').reduce((s: number, x: FirmInvoice) => s + (x.amount - x.paid), 0);
