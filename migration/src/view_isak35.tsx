...
function ISAK35View() {
  ...
  const sc = (n: any) => { if (n == null) return ''; const x = n / U.div; const a = fmt(Math.abs(x), U.dp); return x < 0 ? '(' + a + ')' : a; };
  const M = (n: any) => 'Rp ' + fmt(n / 1e9, 1) + ' M';
  const f0 = (n: any) => fmt(n / 1e6, 0);

  const passed = m.passed;
  const discDone = disc.filter((d: any) => d.done).length;
  ...
  const title = (I35_TITLES as any)[tab];
          ...
          <div className="row gap12" style={{ alignItems: 'flex-start' }}>
              ...
              <Panel noBody>
                ...
                <div style={{ padding: 12, display: 'grid', gap: 12 }}>
                  ...
                  <label className="row ac jb" style={{ cursor: 'pointer' }}>
                    ...
                    <input type="checkbox" checked={comparative} onChange={(e: any) => setComparative(e.target.checked)} />
                  </label>
                  <div>
            ...
            <div style={{ flex: 1, minWidth: 0, background: '#e7eaef', borderRadius: 6, padding: 16, border: '1px solid var(--line)' }}>
              <div className="doc-paper" style={{ background: '#fff', maxWidth: 1080, margin: '0 auto', padding: '46px 66px', boxShadow: 'var(--shadow)', fontSize: 12, color: '#16242c' }}>
                <div style={{ textAlign: 'center', marginBottom: 3, fontWeight: 800, fontSize: 15 }}>{m.entity.name}</div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    ...
                    <tbody>
                      {tab === 'posisi' && <>
                        ...
                        {m.bs.ca.map((l: any) => <R key={l.code} label={l.label} cy={l.cy} py={l.py} note={l.note} />)}
                        <R label="Total Aset Lancar" cy={m.bs.totalCA.cy} py={m.bs.totalCA.py} lvl="subtotal" />
                        <R label="Aset Tidak Lancar" lvl="sub" />
                        {m.bs.nca.map((l: any) => <R key={l.code} label={l.label} cy={l.cy} py={l.py} note={l.note} />)}
                        ...
                        {m.bs.cl.map((l: any) => <R key={l.code} label={l.label} cy={l.cy} py={l.py} note={l.note} />)}
                        ...
                        {m.bs.ncl.map((l: any) => <R key={l.code} label={l.label} cy={l.cy} py={l.py} note={l.note} />)}
                        <R label="Total Liabilitas Jangka Panjang" cy={m.bs.totalNCL.cy} py={m.bs.totalNCL.py} lvl="subtotal" />
                        <R label="Aset Neto" lvl="sub" />
                        {m.bs.naUn.map((l: any) => <R key={l.code} label={l.label} cy={l.cy} py={l.py} note={l.note} />)}
                        {m.bs.naRe.map((l: any) => <R key={l.code} label={l.label} cy={l.cy} py={l.py} note={l.note} />)}
                      ...
                      {tab === 'penghasilan' && <>
                        ...
                        {a.incomeUn.map((l: any) => <R key={l.key} label={l.label} cy={l.cy} py={l.py} note={l.note} indent />)}
                        ...
                        {(expenseView === 'sifat' ? i35ExpenseByNature(a.expense) : a.expense).map((l: any) => <R key={l.key} label={l.label} cy={-l.cy} py={-l.py} note={l.note} indent />)}
                        ...
                        {a.incomeRe.map((l: any) => <R key={l.key} label={l.label} cy={l.cy} py={l.py} note={l.note} indent />)}
                        <R label={a.release.label} cy={-a.release.cy} py={-a.release.py} note="12" indent />
                      ...
                      {tab === 'aruskas' && <>
                        ...
                        {m.cf.cfo.map((l: any, i: any) => l.head ? <R key={i} label={l.label} lvl="sub" /> : <R key={i} label={l.label} cy={l.v} indent={!/Perubahan aset neto/.test(l.label)} />)}
                        ...
                        {m.cf.cfi.map((l: any, i: any) => <R key={'i' + i} label={l.label} cy={l.v} indent />)}
                        ...
                        {m.cf.cff.map((l: any, i: any) => <R key={'f' + i} label={l.label} cy={l.v} indent />)}
...
function i35yoy(o: any, fmt: any) { const d = (o.cy - o.py) / Math.abs(o.py || 1) * 100; return (d >= 0 ? '+' : '') + fmt(d, 1) + '%'; }

/* beban menurut sifat (ilustratif, total identik dgn fungsional) */
function i35ExpenseByNature(expense: any) {
  const tot = expense.reduce((a: any, e: any) => a + e.cy, 0);
  const totPy = expense.reduce((a: any, e: any) => a + e.py, 0);
  const split = [
  ...
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
      <thead>
        <tr style={{ borderBottom: '1.5px solid #2a3f4a' }}>
          <th style={{ textAlign: 'left', padding: '4px 0' }}></th>
          <th style={{ textAlign: 'right', padding: '4px 0', fontSize: 11, width: 150 }}>Tanpa Pembatasan</th>
          <th style={{ textAlign: 'right', padding: '4px 0', fontSize: 11, width: 150 }}>Dengan Pembatasan</th>
          <th style={{ textAlign: 'right', padding: '4px 0', fontSize: 11, width: 130 }}>Total Aset Neto</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((r, i) => (
...
function I35Calk({ m, f0, disc }: any) {
  const e = m.entity;
  const get = (grp: any, code: any) => m.bs[grp].find((l: any) => l.code === code);
  const note = (no: any, title: any, body: any, std?: any) => (
    <div style={{ marginBottom: 13 }}>
      <div className="row ac gap8" style={{ marginBottom: 4 }}>
        <span style={{ fontWeight: 700, fontSize: 12, color: '#0c2430' }}>{no}. {title}</span>
        {std && <span style={{ fontSize: 11, fontWeight: 700, color: '#005085', background: '#e3eef6', padding: '1px 6px', borderRadius: 9 }}>{std}</span>}
      </div>
  ...
  return (
    <div>
      ...
      <div className="row ac jb" style={{ marginTop: 8, padding: '8px 11px', background: '#f3f6f9', borderRadius: 6 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: done === disc.length ? '#1f7a4d' : '#9a6a00', whiteSpace: 'nowrap', marginLeft: 10 }}>{done}/{disc.length}</span>
      </div>
    </div>
  );
}
...
function I35Disclosure({ disc, setDisc }: any) {
  const toggle = (id: any) => setDisc((list: any) => list.map((d: any) => d.id === id ? { ...d, done: !d.done } : d));
  const done = disc.filter((d: any) => d.done).length;
  return (
...
function I35Audit({ proc, done, setDone, nav }: any) {
  const toggle = (i: any) => setDone((d: any) => ({ ...d, [i]: !d[i] }));
  const n = proc.filter((_: any, i: any) => done[i]).length;
  return (
    <div style={{ padding: 12 }}>
      <div className="row ac jb" style={{ marginBottom: 8 }}>
