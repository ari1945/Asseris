...
function mtSystemTasks(audit: any) {
  ...
  aje.filter((a: any) => a.status === 'Proposed').forEach((a: any) => out.push({
...
function useMyTasks() {
  ...
  const tasks = useMemoMT(() => {
    const decorate = (base: any) => {
      ...
      const subDone = sub.filter((s: any) => s.done).length;
    ...
    return [...system.map(decorate), ...personal.map((p: any) => decorate({ ...p, personal: true }))];
  }, [system, personal, meta, engId]);

  const update = (id: any, patch: any) => setMeta((m: any) => ({ ...m, [id]: { ...(m[id] || {}), ...patch } }));
  const setStatus = (id: any, status: any) => update(id, { status });
  const cycleDone = (id: any, cur: any) => update(id, { status: cur === 'done' ? 'todo' : 'done' });
  const toggleStar = (id: any, cur: any) => update(id, { starred: !cur });
  const setSub = (id: any, sub: any) => update(id, { sub });
  const setNote = (id: any, note: any) => update(id, { note });
  const addPersonal = (t: any) => setPersonal((list: any) => [{ id: 'p-' + Date.now(), created: 'baru saja', ...t }, ...list]);
  const removePersonal = (id: any) => { setPersonal((list: any) => list.filter((p: any) => p.id !== id)); setMeta((m: any) => { const n = { ...m }; delete n[id]; return n; }); };

  return { tasks, meta, update, setStatus, cycleDone, toggleStar, setSub, setNote, addPersonal, removePersonal };
}

/* due chip text */
function mtDueLabel(t: any) {
...
function TaskCheck({ status, onClick, size = 18 }: any) {
  return (
    <button className={'mt-check ' + status} onClick={(e: any) => { e.stopPropagation(); onClick(); }} title="Tandai selesai"
  ...
  return (
    <div className={`mt-row p-${t.priority} ${selected ? 'sel' : ''} ${t.done ? 'is-done' : ''}`} onClick={() => onSelect(t.id)}>
      ...
      <button className={'mt-star ' + (t.starred ? 'on' : '')} title="Bintangi"
        onClick={(e: any) => { e.stopPropagation(); mt.toggleStar(t.id, t.starred); }}>
