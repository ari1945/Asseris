          ...
          {groups.map((g) => (
            <div key={g.risk.riskId} style={{ marginBottom: 14 }}>
              ...
              {g.sugs.map((s: Tpl, i: number) => {
                ...
                return (
                  /* Tahap 9 — baris pilihan prosedur: checkbox NATIVE di dalam
                     <label>, bukan div onClick + checkbox readOnly. Klik teks
                     dan input sama-sama meng-toggle; keyboard (Space) bekerja. */
                  <label key={k} className={'prg-sug' + (sel[k] ? ' on' : '')} style={{ cursor: 'pointer' }}>
                    <input type="checkbox" checked={!!sel[k]} onChange={() => toggle(k)} style={{ marginTop: 2, accentColor: 'var(--blue)' }} />
                    <div style={{ flex: 1 }}>
