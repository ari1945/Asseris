...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
function amsCrossChecks(ctx: any) {
  const aje = ctx.aje || [];
  ...
  const fraudRisks = risks.filter((r: any) => r.fraud && /Significant/i.test(r.inherent || ''));
  const fraudAje = aje.filter((a: any) => a.status === 'Proposed' && /fiktif|fraud|channel|override|berelasi|dini/i.test(a.desc));
  if (fraudRisks.length && fraudAje.length) {
    const amt = fraudAje.reduce((s: any, a: any) => s + (a.amount || 0), 0);
    out.push({
      id: 'fraud-aje', sev: 'high', std: 'SA 240 · SA 450',
      title: 'Risiko fraud signifikan vs AJE koreksi belum dibukukan',
      detail: `RoMM fraud terbuka (${fraudRisks.map((r: any) => r.area).join(', ')}) sementara ${fraudAje.length} AJE bermuatan fraud masih Proposed — ${fraudAje.map((a: any) => a.id).join(', ')} senilai ${aiRp(amt)}. Opini tidak dapat difinalisasi sebelum keputusan posting/justifikasi terdokumentasi.`,
  ...
  const invOpen = invArea ? invArea.procs.filter((p: any) => p.exc > 0 || p.status !== 'done') : [];
  if (invPct >= 8 && invOpen.length) {
    const exc = invOpen.reduce((s: any, p: any) => s + (p.exc || 0), 0);
    out.push({
      id: 'inv-nrv', sev: exc > 0 ? 'high' : 'med', std: 'PSAK 14 · SA 540',
      title: 'Persediaan naik YoY namun uji NRV belum tuntas',
      detail: `Persediaan adjusted ${aiRp(invAdj)} — naik ${invPct.toFixed(0)}% dari ${aiRp(invLy)} (audited LY). Uji NRV/keusangan (${invArea.procs.map((p: any) => p.wp).join(', ')}) masih ${exc > 0 ? exc + ' pengecualian terbuka' : 'belum selesai'}. Flag analitis perlu ditindaklanjuti sebelum penilaian persediaan disimpulkan.`,
  ...
  const jetExc = overrideArea ? overrideArea.procs.reduce((s: any, p: any) => s + (p.exc || 0), 0) : 0;
  const overrideRisk = risks.find((r: any) => /override/i.test(r.area) && /Significant/i.test(r.inherent || ''));
  ...
  const noReply = confs.filter((c: any) => c.status === 'No Reply' && (c.days || 0) >= 30);
  if (noReply.length) {
    out.push({
      ...
      detail: `${noReply.map((c: any) => `${c.party.replace(/^PT |^CV /, '')} (${c.days} hr)`).join(', ')}. Untuk non-respons konfirmasi positif, SA 505 mewajibkan prosedur alternatif (mis. subsequent receipt / vouching) — status penyelesaiannya perlu dipastikan.`,
      modules: ['confirm'], refs: ['confirm', 'sad'],
    });
  }

  /* 5) Diskrepansi konfirmasi ⨯ rekonsiliasi ke SAD/ECL */
  const discrep = confs.filter((c: any) => c.status === 'Discrepancy');
  if (discrep.length) {
    const arDisc = discrep.filter((c: any) => c.type === 'Piutang');
    out.push({
      id: 'conf-discrep', sev: 'med', std: 'SA 505 · SA 450',
      title: `${discrep.length} diskrepansi konfirmasi belum tuntas direkonsiliasi`,
      detail: `${discrep.map((c: any) => `${c.party.replace(/^PT |^CV /, '')} (selisih ${aiRp(Math.abs((c.amount || 0) - (c.resp || 0)))})`).join('; ')}. Selisih yang tak terjelaskan harus mengalir ke SAD Ledger${arDisc.length ? ' dan dipertimbangkan terhadap kecukupan ECL piutang' : ''}.`,
  ...
  return out.sort((a, b) => (AI_SEV as any)[b.sev].rank - (AI_SEV as any)[a.sev].rank);
}
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
function useAiInsights(scope: any) {
  ...
  const decide = (ins: any, verdict: any, reason: any) => {
    const when = new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
    setDecisions((d: any) => ({ ...d, [ins.id]: { verdict, who: USER.name, role: USER.role, when, reason: reason || '' } }));
  ...
  return { insights, all, decisions, decide, openCount: insights.filter((i: any) => !decisions[i.id]).length };
}
...
function AiInsChips({ refs, onOpen }: any) {
  return (
    <div className="aiins-refs">
      ...
      {refs.map((id: any) => {
        const m = (MODULE_INDEX as any)[id] || { label: id, icon: 'panel' };
        const RI = (I as any)[m.icon] || I.panel;
...
function AiInsightCard({ ins, decision, onDecide, onOpen }: any) {
  ...
  const sev = (AI_SEV as any)[ins.sev];
        ...
        <div className="aiins-gate">
          ...
          {mode !== 'dismiss' ? (
            ...
            <div className="aiins-dismiss-box">
              <textarea className="aiins-input" rows={2} value={reason} onChange={(e: any) => setReason(e.target.value)} placeholder="Alasan diabaikan / pertimbangan auditor (wajib dicatat)…" />
              <div className="aiins-actions">
...
function AiInsightPanel({ scope, title, embedded }: any) {
  ...
  const open = insights.filter((i: any) => !decisions[i.id]);
  const done = insights.filter((i: any) => decisions[i.id]);
  ...
  return (
    <div className={'aiins-wrap' + (embedded ? ' embed' : '')}>
      <div className="aiins-head">
        ...
        <div style={{ flex: 1 }} />
        {done.length > 0 && <button className="aiins-toggle" onClick={() => setShowDone((s: any) => !s)}>{showDone ? 'Sembunyikan diputuskan' : `Tampilkan ${done.length} diputuskan`}</button>}
      </div>
