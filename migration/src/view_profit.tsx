...
import {
  pmExtraHours, pmPartners, pmRecovery, pmRecoveryTotals, pmRows, pmTotals,
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} className={r.id === sel ? 'sel' : ''} onClick={() => setSel(r.id)} style={{ cursor: 'pointer', opacity: r.incomplete ? 0.72 : 1 }}>
                      ...
                      <td className="num muted">{jt(r.stdCost)}</td>
...
const GRADE_COST: Readonly<Record<string, number>> = { Partner: 2_500_000, Manager: 1_200_000, Senior: 700_000, Junior: 400_000 };
...
function LeverageRecovery({ rows, fmt, marginColor }: LevProps) {
  ...
  const rec: PMRecoveryRow[] = pmRecovery(rows, CHARGE_MULT);
    ...
    <div className="grid split" style={{ gridTemplateColumns: '360px 1fr', gap: 12, alignItems: 'start' }}>
      ...
      <Panel noBody>
        ...
        <table className="dtbl">
          ...
          <tbody>
            {rec.map((r) => (
              <tr key={r.id} style={{ opacity: r.incomplete ? 0.72 : 1 }}>
                ...
                <td className="num" style={{ fontWeight: 600, color: r.writedown === null ? 'var(--ink-2)' : r.writedown > 0 ? 'var(--red)' : 'var(--green)' }}>{r.writedown === null ? NA : r.writedown > 0 ? '(' + fmt(r.writedown / 1e6, 0) + ')' : '+' + fmt(-r.writedown / 1e6, 0)}</td>
          ...
          <tfoot><tr><td colSpan={3}>TOTAL ({rec.length - t.incomplete.length}/{rec.length})</td><td className="num">{fmt(t.wip / 1e6, 0)}</td><td className="num">{fmt(t.billed / 1e6, 0)}</td><td className="num" style={{ color: 'var(--red)' }}>({fmt(t.writedown / 1e6, 0)})</td><td className="num">{NAp(t.recoveryPct)}</td></tr></tfoot>
        </table>
        <div className="tiny muted" style={{ padding: '8px 12px', lineHeight: 1.5 }}>WIP charge-out = jam × tarif standar (cost × {CHARGE_MULT}). Selisih terhadap fee terealisasi adalah <b>write-up</b> (fee &gt; standar) atau <b>write-down</b> (penghapusan WIP tak tertagih). Recovery rata-rata firma <b>{NAp(t.recoveryPct)}</b>{t.incomplete.length > 0 && <> — dihitung dari {rec.length - t.incomplete.length} dari {rec.length} engagement; sisanya tak punya fee/tarif realisasi tercatat</>}.</div>
      </Panel>
    </div>
  );
}
