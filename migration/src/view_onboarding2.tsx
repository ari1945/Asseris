  ...
  const sign = async () => {
    ...
    try {
      const sealResult = await amsExportPdf({
        ...
        /* Surat perikatan ditandatangani saat perikatannya BELUM ada — prospek. */
        scope: 'firm',
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        <div className="panel" style={{ padding: 14 }}>
          ...
          <div style={{ display: 'grid', gap: 7, marginTop: 12 }}>
            ...
            {L.status === 'signed' && <Btn sm onClick={() => amsExportPdf({ kind: 'engagement-letter', scope: 'firm', fileName: `Surat Perikatan - ${p.name}.pdf`, title: 'Surat Perikatan Audit', refNo: `No. ${p.id}/EL/${new Date(today).getFullYear()}`, meta: [`${p.id} · ${p.name} · ${p.standard}`, `Versi ${L.version}`], blocks: buildLetterPayload(p, { ...L, signedBy: L.signedBy || (p.name + ' (Direksi)'), signedDate: L.signedDate || today }) }).catch(() => {})}><I.download size={12} /> Cetak / Export PDF</Btn>}
