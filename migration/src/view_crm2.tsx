...
function CRM360() {
  ...
  const withData = clients.filter((c: any) => C360[c.id]);
      ...
      <div className="grid" style={{ gridTemplateColumns: '270px 1fr', gap: 12, alignItems: 'start' }}>
        ...
        <Panel noBody>
          ...
          <div style={{ maxHeight: 560, overflow: 'auto' }}>
            {withData.sort((a: any, b: any) => (C360[b.id].health) - (C360[a.id].health)).map((c: any) => {
              const ch = C360[c.id];
              return (
                ...
                {(h.activities || []).map((a: any, i: any) => {
                  ...
                  return (
                    <div key={i} className="row gap10" style={{ padding: '9px 0', borderBottom: i < h.activities.length - 1 ? '1px solid var(--line-soft)' : 0 }}>
                      <span style={{ width: 28, height: 28, borderRadius: 7, background: m.color, color: '#fff', display: 'grid', placeItems: 'center', flex: '0 0 28px' }}>{React.createElement((I as any)[m.icon] || I.doc, { size: 14 })}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
...
function CRMAktivitas() {
  ...
  const feed: any[] = [];
  clients.forEach((c: any) => { (C360[c.id]?.activities || []).forEach((a: any) => feed.push({ ...a, client: c.name.replace('PT ', ''), clientId: c.id })); });
...
function CRMSegmentasi() {
  ...
  const seg = (keyFn: any) => {
    const m = {};
    clients.forEach((c: any) => { const k = keyFn(c); if (!(m as any)[k]) (m as any)[k] = { k, n: 0, fee: 0 }; (m as any)[k].n++; (m as any)[k].fee += c.fee; });
    return (Object.values(m) as any[]).sort((a, b) => b.fee - a.fee);
  };
  const byIndustry = seg((c: any) => c.industry.split(' · ')[0]);
  const byTier = seg((c: any) => c.tier);
  const totFee = clients.reduce((s: any, c: any) => s + c.fee, 0);
  ...
  const cell = (t: any, r: any) => clients.filter((c: any) => c.tier === t && c.risk === r);
  const healthVals = clients.filter((c: any) => C360[c.id]).map((c: any) => C360[c.id].health);
  const avgHealth = Math.round(healthVals.reduce((s: any, v: any) => s + v, 0) / (healthVals.length || 1));
              ...
              <div style={{ flex: 1 }}>
                {byTier.map(t => (
                  <div key={t.k} className="row jb ac" style={{ padding: '4px 0' }}>
                    <span className="row ac gap6"><span style={{ width: 9, height: 9, borderRadius: 2, background: (tierColor as any)[t.k] || '#9aa7b2' }} /><span style={{ fontSize: 12, fontWeight: 600 }}>{t.k}</span></span>
      ...
      <Panel title="Matriks Tier × Risiko" sub="jumlah klien & fee per sel">
        <div style={{ padding: 14, overflowX: 'auto' }}>
          <table className="dtbl" style={{ minWidth: 480 }}>
            <thead><tr><th style={{ width: 90 }}>Tier \ Risiko</th>{risks.map(r => <th key={r} className="num"><span className="row ac gap6 je"><span style={{ width: 9, height: 9, borderRadius: 2, background: (riskColor as any)[r] }} />{r}</span></th>)}<th className="num">Total</th></tr></thead>
            <tbody>
              {tiers.map(t => {
                const rowClients = clients.filter((c: any) => c.tier === t);
                return (
                  <tr key={t}>
                    <td style={{ fontWeight: 700 }}>{t}</td>
                    {risks.map(r => {
                      const cs = cell(t, r);
                      const fee = cs.reduce((s: any, c: any) => s + c.fee, 0);
            ...
            <tfoot><tr><td>Total</td>{risks.map(r => <td key={r} className="num">{clients.filter((c: any) => c.risk === r).length}</td>)}<td className="num">{clients.length}</td></tr></tfoot>
          </table>
        </div>
      </Panel>
    </div></div>
