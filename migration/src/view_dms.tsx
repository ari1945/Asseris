...
function UploadModal({ onClose, onAdd }: any) {
  ...
  const onFiles = (metas: any) => {
    setFiles(metas);
    const first = metas.find((m: any) => m.ok) || metas[0];
  ...
  return (
    <PModal icon="upload" title="Unggah Dokumen ke DMS" sub="Terenkripsi AES-256 · klasifikasi & retensi otomatis sesuai ISQM" onClose={onClose} width={560}
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <PField label="Nama Dokumen" span><input className="input" value={f.name} onChange={(e: any) => set('name', e.target.value)} placeholder="mis. Kertas Kerja — Sentosa Makmur FY2025" autoFocus /></PField>
...
function DocDrawer({ d, onClose, onToggleHold, onAccess, fmt }: any) {
  ...
  const toggleCmp = (ver: any) => setCmp((list: any) => list.includes(ver) ? list.filter((v: any) => v !== ver) : [...list, ver].slice(-2));
  const cmpVers = cmp.map((v: any) => d.versions.find((x: any) => x.ver === v)).filter(Boolean).sort((x: any, y: any) => x.ver - y.ver);
    ...
    <PDrawer open={!!d} onClose={onClose} width={580}>
      <div className="pdrawer-h">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="row ac gap6 wrap" style={{ marginTop: 7 }}>
            ...
            <Badge kind={(ASSEMBLY as any)[d.assembly].k}>{(ASSEMBLY as any)[d.assembly].l}</Badge>
      ...
      <div className="pdrawer-body">
        ...
        {(() => {
          ...
          const SecRow = ({ ic, label, value, ok = true }: any) => (
            <div className="row ac gap8" style={{ padding: '6px 0', borderBottom: '1px solid var(--line-soft)' }}>
              <span style={{ color: ok ? 'var(--green)' : 'var(--amber)', flex: '0 0 16px' }}>{React.createElement((I as any)[ic] || I.circle, { size: 14 })}</span>
        ...
        {sub === 'akses' && (
          <div className="ptl">
            ...
            {(d.access || []).slice().reverse().map((ac: any, i: any) => {
              const meta = (PEVT as any)[ac[1]] || { ic: 'circle', c: 'var(--ink-4)' };
              const IconA = (I as any)[meta.ic] || I.circle;
              return (
                <div key={i} className="ptl-row">
                  <div className="ptl-mark" style={{ color: meta.c, borderColor: meta.c }}><IconA size={11} /></div>
                  <div className="ptl-body"><div style={{ fontSize: 12, fontWeight: 600 }}>{(ACCESS_LBL as any)[ac[1]] || ac[1]} <span className="muted" style={{ fontWeight: 500 }}>oleh {ac[0]}</span></div><div className="tiny muted mono">{ac[2]}</div></div>
                </div>
              );
            })}
          </div>
...
function DocManagement() {
  ...
  const assembling = docs.filter((d: any) => d.assembly === 'in-progress' || d.assembly === 'pending');
  const totalGB = (docs.reduce((s: any, d: any) => s + d.sizeMB, 0) / 1024);
  const expiringSoon = docs.filter((d: any) => { const rr = retentionInfo(d); return rr && !rr.expired && rr.yrsLeft < 1; }).length;

  /* folders by engagement */
  const folders = useMemoDMS(() => {
    const map = {};
    docs.forEach((d: any) => { ((map as any)[d.eng] = (map as any)[d.eng] || { eng: d.eng, client: d.client, count: 0 }).count++; });
  ...
  const byFolder = (d: any) => folder === 'all' || d.eng === folder;
  const filtered = docs.filter((d: any) => byFolder(d) && (q === '' || d.name.toLowerCase().includes(q.toLowerCase()) || d.eng.toLowerCase().includes(q.toLowerCase()) || (d.client || '').toLowerCase().includes(q.toLowerCase())));
  const sel = docs.find((d: any) => d.id === selId);
        ...
        <div className="grid" style={{ gridTemplateColumns: '212px 1fr', gap: 12, alignItems: 'start' }}>
          ...
          <Panel noBody>
            <div className="panel-h" style={{ padding: 0, background: 'var(--surface-2)' }}>
              ...
              <div style={{ flex: 1 }} />
              <div className="global-search" style={{ background: 'var(--surface)', border: '1px solid var(--line)', height: 26, maxWidth: 180, margin: '0 10px' }}><I.search2 size={13} style={{ color: 'var(--ink-4)' }} /><input style={{ color: 'var(--ink)' }} placeholder="Cari dokumen…" value={q} onChange={(e: any) => setQ(e.target.value)} /></div>
            </div>
                  ...
                  {filtered.map((d: any) => {
                    ...
                    return (
                      <tr key={d.id} onClick={() => setSelId(d.id)} style={{ cursor: 'pointer' }} className={d.id === selId ? 'sel' : ''}>
                        ...
                        <td><Badge kind={(CLASS_KIND as any)[d.classification]}>{d.classification}</Badge></td>
                        ...
                        <td><Badge kind={(ASSEMBLY as any)[d.assembly].k}>{(ASSEMBLY as any)[d.assembly].l}</Badge></td>
                      </tr>
                    );
                  })}
            ...
            {tab === 'assembly' && (
              <div style={{ padding: 14, display: 'grid', gap: 10 }}>
                {assembling.filter(byFolder).length ? assembling.filter(byFolder).map((d: any) => {
                  const a = assemblyInfo(d);
                  return (
                  ...
                  {docs.filter((d: any) => d.archivedOn && byFolder(d)).map((d: any) => {
                    ...
                    return (
                      <tr key={d.id} onClick={() => setSelId(d.id)} style={{ cursor: 'pointer' }} className={d.id === selId ? 'sel' : ''}>
                        ...
                        <td>{rr.expired && !d.legalHold ? <button className="btn sm" style={{ height: 22, color: 'var(--red)' }} onClick={(e: any) => e.stopPropagation()}><I.x size={11} /> Disposal</button> : d.legalHold ? <span className="tiny" style={{ color: 'var(--red)' }}><I.lock size={10} /> Ditahan</span> : <span className="tiny muted">Aktif</span>}</td>
                      </tr>
                    );
                  })}
                </tbody>
              ...
              <div style={{ padding: 14 }}>
                {onHold.filter(byFolder).length ? (
                  <div style={{ display: 'grid', gap: 10 }}>
                    {onHold.filter(byFolder).map((d: any) => (
