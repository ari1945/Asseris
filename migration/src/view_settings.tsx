...
const SSelect = ({ value, onChange, options, w = 200 }: any) => (
  <select className="select" value={value} onChange={(e: any) => onChange(e.target.value)} style={{ width: w, height: 30 }}>
...
function SettingsView() {
  ...
  const setGroup = (grp: any, key: any, val: any) => setSettings((p: any) => ({ ...p, [grp]: { ...(p[grp] || (SETTINGS_DEFAULTS as any)[grp]), [key]: val } }));
  const setTop = (key: any, val: any) => setSettings((p: any) => ({ ...p, [key]: val }));
    ...
    <>
      ...
      <Panel noBody>
        ...
        <SRow title="Kurangi Animasi" sub="Menonaktifkan transisi & animasi untuk kenyamanan visual." last>
          <SetToggle on={s.reduceMotion} set={(v: any) => setTop('reduceMotion', v)} />
        </SRow>
      </Panel>
    </>
  );
...
function SecNavigasi({ s, setGroup }: any) {
  const n = s.nav;
  const set = (k: any, v: any) => setGroup('nav', k, v);
    ...
    <>
      <Panel noBody>
        ...
        <SRow title="Aktifkan sidebar adaptif" sub="Sidebar menyesuaikan urutan & penekanan modul dengan fase audit yang sedang berjalan. Saat dimatikan, sidebar kembali ke perilaku standar (semua grup setara).">
          <SetToggle on={n.adaptive} set={(v: any) => set('adaptive', v)} />
        </SRow>
        <div style={dim}>
          <SRow title="Mode penyesuaian" sub="Sorot — semua modul tetap tampil, yang tak relevan diredupkan. Ringkas — grup fase lain disembunyikan demi fokus maksimal.">
            <Seg options={[{ value: 'sorot', label: 'Sorot' }, { value: 'ringkas', label: 'Ringkas' }]} value={n.mode} onChange={(v: any) => set('mode', v)} />
          </SRow>
          <SRow title="Sumber fase aktif" sub="Otomatis membaca fase dari engagement aktif, atau pilih manual untuk mengunci fokus." last={n.source !== 'manual'}>
            <SSelect value={n.source === 'manual' ? 'Manual' : 'Otomatis dari engagement'} onChange={(v: any) => set('source', v === 'Manual' ? 'manual' : 'auto')} options={['Otomatis dari engagement', 'Manual']} w={210} />
          </SRow>
          {n.source === 'manual' && (
            <SRow title="Fase manual" sub="Fokus dikunci ke fase ini, terlepas dari status engagement aktif." last>
              <SSelect value={n.manualPhase} onChange={(v: any) => set('manualPhase', v)} options={['Perencanaan', 'Eksekusi', 'Finalisasi']} w={170} />
            </SRow>
          )}
        </div>
      </Panel>
      ...
      <div style={dim}>
        <Panel noBody>
          ...
          <SRow title="Grup “Fokus Fase”" sub="Mengangkat modul paling relevan untuk fase aktif ke puncak sidebar dalam satu grup tersorot.">
            <SetToggle on={n.focusGroup} set={(v: any) => set('focusGroup', v)} />
          </SRow>
          <SRow title="Kartu “Lanjutkan”" sub="Jalan cepat kembali ke modul yang terakhir dibuka.">
            <SetToggle on={n.resumeCard} set={(v: any) => set('resumeCard', v)} />
          </SRow>
          <SRow title="Jumlah item “Lanjutkan”" sub="Berapa modul terakhir yang ditampilkan.">
            <Seg options={[{ value: 2, label: '2' }, { value: 3, label: '3' }, { value: 5, label: '5' }]} value={n.resumeCount} onChange={(v: any) => set('resumeCount', v)} />
          </SRow>
          <SRow title="Tandai fase selesai" sub="Beri centang pada modul dari fase yang sudah rampung di pohon penuh." last>
            <SetToggle on={n.markDone} set={(v: any) => set('markDone', v)} />
          </SRow>
        </Panel>
      </div>
...
function readAvatarFile(file: any, max: any, cb: any) {
    ...
    <>
      <Panel noBody>
        ...
        <div style={{ padding: 16 }}>
          <div className="row gap16" style={{ marginBottom: 18, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: '0 0 auto', textAlign: 'center' }}>
              <div
                ...
                onDrop={(e: any) => { e.preventDefault(); setDrag(false); onFile(e.dataTransfer.files && e.dataTransfer.files[0]); }}
              ...
              <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={(e: any) => { onFile(e.target.files && e.target.files[0]); e.target.value = ''; }} />
          ...
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <ProfileField label="Nama Lengkap" val={u.name} onChange={(v: any) => up({ name: v })} />
            <ProfileField label="Jabatan (Job Title)" val={u.title} onChange={(v: any) => up({ title: v })} hint="Gelar fungsional di firma — terpisah dari Peran/RBAC." />
            <ProfileField label="Email Kerja" type="email" val={u.email} onChange={(v: any) => up({ email: v })} />
            <ProfileField label="Telepon" val={u.phone} onChange={(v: any) => up({ phone: v })} />
            <ProfileField label="Departemen / Lini Jasa" val={u.department} onChange={(v: any) => up({ department: v })} />
            <ProfileField label="Kantor" val={u.office} onChange={(v: any) => up({ office: v })} />
            <ProfileField label="ID Karyawan" val={u.employeeId} readOnly mono hint="Dikelola HR — hanya-baca." />
            <ProfileField label="Bahasa" val={u.languages} onChange={(v: any) => up({ languages: v })} />
          </div>
      ...
      <Panel noBody>
        ...
        <SRow title="Inisial Tanda Tangan" sub="Dipakai pada sign-off kertas kerja & tickmark." last>
          <input className="input" value={u.initials} onChange={(e: any) => up({ initials: e.target.value.toUpperCase().slice(0, 3) })} style={{ width: 80, height: 30, textAlign: 'center', fontWeight: 700, letterSpacing: '.05em' }} />
        </SRow>
        <div style={{ padding: '0 16px 16px' }}>
    ...
    <>
      <Panel noBody>
        ...
        <SRow title="Email" sub="Kirim notifikasi ke alamat email kerja.">
          <SetToggle on={n.email} set={(v: any) => setGroup('notif', 'email', v)} />
        </SRow>
        <SRow title="Dalam Aplikasi" sub="Lonceng notifikasi & panel aktivitas.">
          <SetToggle on={n.app} set={(v: any) => setGroup('notif', 'app', v)} />
        </SRow>
        <SRow title="Push (Mobile)" sub="Notifikasi dorong ke aplikasi seluler Asseris." last>
          <SetToggle on={n.push} set={(v: any) => setGroup('notif', 'push', v)} />
        </SRow>
      </Panel>
      ...
      <Panel noBody>
        ...
        {cats.map(([key, label, sub], i) => (
          <SRow key={key} title={label} sub={sub} last={i === cats.length - 1}>
            <SetToggle on={n[key]} set={(v: any) => setGroup('notif', key, v)} />
          </SRow>
        ))}
      </Panel>
      ...
      <Panel noBody>
        ...
        <SRow title="Frekuensi Ringkasan" sub="Email rangkuman aktivitas perikatan.">
          <SSelect value={n.digest} onChange={(v: any) => setGroup('notif', 'digest', v)} options={['Real-time', 'Harian', 'Mingguan', 'Nonaktif']} w={160} />
        </SRow>
        <SRow title="Waktu Tenang (Quiet Hours)" sub="Hentikan notifikasi non-kritis pada rentang waktu ini." last>
          <div className="row ac gap6">
            <input className="input mono" type="time" value={n.quietStart} onChange={(e: any) => setGroup('notif', 'quietStart', e.target.value)} style={{ width: 110, height: 30 }} />
            <span className="muted tiny">s/d</span>
            <input className="input mono" type="time" value={n.quietEnd} onChange={(e: any) => setGroup('notif', 'quietEnd', e.target.value)} style={{ width: 110, height: 30 }} />
          </div>
        </SRow>
      </Panel>
    </>
    ...
    <>
      <Panel noBody>
        ...
        <SRow title="Batas Waktu Sesi" sub="Berlaku di server (SESSION_TTL_HOURS); pilihan ini bersifat preferensi tampilan.">
          <SSelect value={sec.sessionTimeout} onChange={(v: any) => setGroup('security', 'sessionTimeout', v)} options={['15 menit', '30 menit', '1 jam', '4 jam']} w={140} />
        </SRow>
        <SRow title="Peringatan Login" sub="Email saat ada login dari perangkat/lokasi baru (rencana W10).">
          <SetToggle on={sec.loginAlerts} set={(v: any) => setGroup('security', 'loginAlerts', v)} />
        </SRow>
        <SRow title="Daftar Izin IP Kantor" sub="Batasi akses hanya dari jaringan kantor terdaftar (rencana W10)." last>
          <SetToggle on={sec.ipAllowlist} set={(v: any) => setGroup('security', 'ipAllowlist', v)} />
        </SRow>
      </Panel>
        ...
        <table className="dtbl">
          ...
          <tbody>
            ...
            {events.map((e: any, i: any) => (
              <tr key={i}>
                <td style={{ fontWeight: 600 }}>{(AUTH_EVENT_LABEL as any)[e.kind] || e.kind}{e.detail ? <span className="tiny muted"> · {e.detail}</span> : null}</td>
    ...
    <>
      ...
      <Panel noBody>
        ...
        <SRow title="Standar Audit Default" sub="Standar yang diterapkan untuk perikatan baru.">
          <SSelect value={f.defaultStandard} onChange={(v: any) => isPartner && setGroup('firm', 'defaultStandard', v)} options={['SA (ISA-converged)', 'SA + PSAK 71', 'SA + PSAK 73', 'SPR 2400']} w={210} />
        </SRow>
        <SRow title="Akhir Tahun Buku" sub="Tanggal pelaporan keuangan default klien.">
          <SSelect value={f.fiscalYearEnd} onChange={(v: any) => isPartner && setGroup('firm', 'fiscalYearEnd', v)} options={['31 Desember', '31 Maret', '30 Juni', '30 September']} w={160} />
        </SRow>
        <SRow title="Benchmark Materialitas" sub="Dasar perhitungan overall materiality.">
          <SSelect value={f.materialityBenchmark} onChange={(v: any) => isPartner && setGroup('firm', 'materialityBenchmark', v)} options={['5% Laba Sebelum Pajak', '0,5% Pendapatan', '1% Total Aset', '1% Ekuitas']} w={210} />
        </SRow>
  ...
  return (
    <Panel noBody>
      ...
      <SRow title="Bahasa" sub="Bahasa antarmuka aplikasi.">
        <SSelect value={l.language} onChange={(v: any) => setGroup('locale', 'language', v)} options={['Bahasa Indonesia', 'English']} w={180} />
      </SRow>
      <SRow title="Format Tanggal">
        <SSelect value={l.dateFormat} onChange={(v: any) => setGroup('locale', 'dateFormat', v)} options={['DD/MM/YYYY', 'DD-MM-YYYY', 'YYYY-MM-DD', 'D MMMM YYYY']} w={170} />
      </SRow>
      <SRow title="Format Angka" sub="Pemisah ribuan & desimal.">
        <SSelect value={l.numberFormat} onChange={(v: any) => setGroup('locale', 'numberFormat', v)} options={['1.234.567,89', '1,234,567.89', '1 234 567,89']} w={170} />
      </SRow>
      <SRow title="Mata Uang Pelaporan">
        <SSelect value={l.currency} onChange={(v: any) => setGroup('locale', 'currency', v)} options={['IDR (Rp)', 'USD ($)', 'SGD (S$)', 'EUR (€)']} w={150} />
      </SRow>
      <SRow title="Zona Waktu">
        <SSelect value={l.timezone} onChange={(v: any) => setGroup('locale', 'timezone', v)} options={['WIB (GMT+7)', 'WITA (GMT+8)', 'WIT (GMT+9)']} w={170} />
      </SRow>
      <SRow title="Hari Pertama Pekan" last>
        <Seg options={['Senin', 'Minggu']} value={l.firstDay} onChange={(v: any) => setGroup('locale', 'firstDay', v)} />
      </SRow>
    </Panel>
  );
}
...
function SecAI({ s, setGroup, flash }: any) {
  ...
  const active = PROVIDERS.find((p: any) => p.id === ai.provider) || PROVIDERS[0] || {};
  const keys = ai.keys || {};
  ...
  useEffectSet(() => {
    ...
    (window.amsLlmStatus ? window.amsLlmStatus() : Promise.resolve({ configured: false }))
      .then((st: any) => { if (live) setSrv({ state: 'ready', ...st }); })
  ...
  const setProvider = (pid: any) => { setGroup('ai', 'provider', pid); setTest(null); setShowKey(false); };
  const setKey = (val: any) => { setGroup('ai', 'keys', { ...keys, [active.id]: val }); setTest(null); };
  const setBase = (val: any) => setGroup('ai', 'baseUrls', { ...baseUrls, [active.id]: val });
    ...
    <>
      ...
      <Panel noBody>
        <div className="panel-h">
          ...
          <div style={{ flex: 1 }} />
          <span className={'chip tiny llms-bbadge ' + browserBadge.cls}>{React.createElement((I as any)[browserBadge.ic], { size: 11 })} {browserBadge.t}</span>
        </div>
      ...
      <Panel noBody>
        ...
        <div style={{ padding: '4px 0' }}>
          <SRow title="Bagikan konteks perikatan" sub="Klien, perikatan aktif, materialitas & modul saat ini disertakan agar jawaban relevan. Matikan untuk pertanyaan umum/anonim.">
            <SetToggle on={ai.share !== false} set={(v: any) => setGroup('ai', 'share', v)} />
          </SRow>
          <SRow title="Gaya keluaran" sub="Presisi mengutamakan jawaban faktual & ringkas; Kreatif lebih eksploratif." last>
            <SSelect value={ai.temperature || 'Standar'} onChange={(v: any) => setGroup('ai', 'temperature', v)} options={AI_TEMPS} w={190} />
          </SRow>
        </div>
