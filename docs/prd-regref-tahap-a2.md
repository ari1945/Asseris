...
| Tanggal | 2026-08-21 |
| Pemilik | Ari Widodo |
| Status | **In Progress** — arahan Ari 2026-08-21 (R1–R4) diperlakukan sebagai lingkup yang disetujui; Tahap B (halaman dapat ditulis admin) TETAP terpisah dan belum diputuskan. **PR-1..PR-5 SELESAI** — SC-A1..SC-A11 tertutup, live-verified pada lima tanggal (2009 · 2016 · 2021 · 2026 · 2027). Q-A1·Q-A2·Q-A3 menunggu DATA dari Ari dan TIDAK memblokir mekanismenya |
...
| Berkas | BARU: `canon_cit.ts` · `regref_census.ts` · `data_clock.ts` · `regref_census.test.ts` · `regref_tahap_a2.test.ts`. DIUBAH: `canon_ppl.ts` · `canon_rotation.ts` · `canon_base.ts` · `canon_part3.ts` · `regref_catalog.ts` · `data_part1.ts` · `data_part4.ts` · `data_licensing.ts` · `data_proforma.ts` · `ethics_compliance.ts` · `view_aje.tsx` · `view_firmtax.tsx` · `view_psak46.tsx` · `view_psak24.tsx` · `view_spr2410.tsx` · `view_newdisc.tsx` · `view_people.tsx` · `view_personal.tsx` · `view_bo3.tsx` |
...
Catatan penegakan: aturan Q-3 (“yang menyangkut uang MEMBLOKIR”) diperluas ke **premis
sebuah kesimpulan**. Tarif pajak & ambang GloBE menyangkut uang; kewajiban PPL dan batas
rotasi menyangkut verdict kepatuhan, yang juga tak punya jawaban separuh. Kalender cuti
tetap satu-satunya `warn`: hari kerja masih berarti tanpa kalendernya, hanya kurang teliti.
...
**PR-2 · PPL berkunci masa** — `PPL_REGISTRY` di `canon_ppl.ts` (`block`), `pplReqOn()`,
`pplYearOf()`, `skpInYear()`, `pplPeriod()`. `CPE_REQ.year` berhenti menjadi sumber tahun
(klok tunggal `data_clock.ts`); catatan SKP disaring ke tahun yang berlaku.

**PR-3 · Rotasi berkunci masa** — `ROTATION_REGISTRY` di `canon_rotation.ts` (`block`),
`rotationRegimesOn()`/`rotationRegimesRequired()`, `regimeOf({ regimes })`. Fallback
`|| 5` dicabut; basis peringatan ditarik dari registry (`view_people`, `view_bo3`).
...
| SC | Bukti |
|---|---|
| SC-A1 | `pplReqOn('2021-12-31')` -> `no-coverage` + `blocked`; `pplReqOn('2026-03-09').value` **`toBe`** `PPL_REQ_PMK186` (identitas objek, bukan salinan) |
| SC-A2 | `CPE_REQ.year` = `pplYearOf(SEED_TODAY)`; gerbang menolak `ROTATION_YEAR = <tahun>`, `CPE_REQ = { ... year: <tahun> }`, dan `TODAY = '<tanggal>'` |
| SC-A3 | `skpInYear` membuang entri tahun lain DAN entri tanpa tanggal terbaca; `pplPeriod` 2026 vs 2025 memberi total berbeda dari data yang sama |
| SC-A4 | Grep `rotationLimit \|\| 5` = nol; AP tanpa deklarasi -> `tak-dinilai`, bukan "Patuh" |
| SC-A5 | `basisOf()` di `view_people`, `a.rotationBasis` di `view_bo3`; gerbang menolak kembalinya string "UU 5/2011 & POJK 13/2017" |
| SC-A6 | 2019 -> **0,25**; 2020/2025 -> 0,22; 2009 -> `no-coverage` + `citRateRequired` melempar |
| SC-A7 | Delapan berkas yang dulu mengetik tarifnya kini nol; detektor memaku JUMLAH per berkas |
| SC-A8 | `globe-min` di katalog; `P2_MIN_RATE = 15.0` dicabut; 2024 `no-coverage` |
| SC-A9 | Ditanam `const PPN_RATE = 0.11;` + satu `0.22` liar -> **dua gerbang merah** dengan pesan yang menyebut berkas & nama |
| SC-A10 | Diganti nama situs terdaftar menjadi hantu -> **dua gerbang prune merah** |
| SC-A11 | 2.807 uji lama hijau tanpa disentuh; snapshot `canon_regression` tak bergeser; `AMS.CPE_REQ`, `AMS.TODAY`, tenure/batas INDEPENDENCE identik |

**Live-verified** (`javascript_tool`, tanpa login): pada 2026-03-09 nol registry berhenti;
2027-01-01 hanya BPJS berhenti (sama seperti Tahap A); 2021-06-30 BPJS + TER + `globe-min`
+ `ppl` berhenti. Dua belas modul yang disentuh diimpor bersih - tak ada konstanta
tingkat-modul yang melempar saat boot.
